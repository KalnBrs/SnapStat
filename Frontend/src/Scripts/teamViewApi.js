...
async function getTeam(team_id) {
  ...
  const response = await fetch(`https://ec2-13-217-114-28.compute-1.amazonaws.com/api/teams/${team_id}`, {
    method: "GET",
    headers: {
...
async function getRoster(team_id) {
  const user = store.getState().user.user
  const response = await fetch(`https://ec2-13-217-114-28.compute-1.amazonaws.com/api/teams/${team_id}/players`, {
    method: "GET",
    headers: {
      "Authorization": `Bearer ${user.accessToken}`,
    }
...
async function getPlayerStats(player_id, game_id) {
  ...
  try {
    const response = await fetch(`https://ec2-13-217-114-28.compute-1.amazonaws.com/api/stats/${player_id}/${game_id}`, {
      method: "GET",
      headers: {
        "Authorization": `Bearer ${user.accessToken}`,
      }
