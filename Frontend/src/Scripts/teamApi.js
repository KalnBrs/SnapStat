...
async function postTeam(team) {
  ...
  const res = await fetch(`https://ec2-13-217-114-28.compute-1.amazonaws.com/api/teams/`, {
    method: "POST",
    headers: {
...
async function clearTeam(team) {
  const user = store.getState().user.user;
  const res = await fetch(`https://ec2-13-217-114-28.compute-1.amazonaws.com/api/teams/${team.team_id}`, {
    method: "DELETE",
    headers: {
      "Authorization": `Bearer ${user.accessToken}`,
    },
